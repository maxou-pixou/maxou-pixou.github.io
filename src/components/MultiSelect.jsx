  ...
  return (
    <div className="relative">
      <button
        ...
        className="min-w-[180px] inline-flex justify-between items-center gap-4 px-2 pr-1 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black transition-all"
      >
          ...
          <ul className="border border-gray-200 divide-y divide-gray-200 list-none w-fit">
            <li className="flex items-center gap-1 text-sm cursor-pointer w-96">
              <div className="relative w-full py-2">
                ...
                <input
                  className="block w-full rounded-md border-gray-300 !pl-10 focus:border-black focus:ring-2 focus:ring-black/20 md:text-sm transition-all"
                  placeholder="Search"
                  value={search}
                  onChange={(e) => {
                    e.persist();
              ...
              .map((option) => {
                ...
                return (
                  <li
                    ...
                    className="flex items-center gap-1 hover:bg-gray-100 text-sm px-2 py-1 cursor-pointer w-96 transition-colors"
                  >
                    {isSelected ? <MdCheckBox className="text-black" /> : <MdCheckBoxOutlineBlank className="text-gray-500" />}
                    <span className={`flex-1 ${isSelected ? "font-semibold text-black" : "font-normal"}`}>{option.label}</span>
                  </li>
                );
              })}
          </ul>
